...
from valheim_log_parser import parse_valheim_log, ValheimSession, LogEntry, handle_message  # Import the dataclass
...
def test_parse_valheim_log():
    ...
    for log_entry, expected_result in test_cases:
        ...
        assert actual_result == expected_result, f"Test failed for input: '{log_entry}'. Expected: {expected_result}, Got: {actual_result}"
    ...
    for log_entry in negative_test_cases:
        actual_result = parse_valheim_log(log_entry)
        assert actual_result is None, f"Negative test failed for input: '{log_entry}'. Expected None, Got: {actual_result}"
...
def test_handle_message_with_callback():
    ...
    test_entry = {"MESSAGE": 'Session "TestServer" with join code 12345 and IP 127.0.0.1:2456 is active with 2 player(s)'}
